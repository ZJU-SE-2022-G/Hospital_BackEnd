...
public class NoticeController {
    ...
    public JsonResultUtil<Notice> getNoticeById(Long id){
        return new JsonResultUtil<>(200,noticeService.getById(id));
    }
    ...
    public JsonResultUtil<Notice> deleteNoticeById(Long id){
        ...
        return new JsonResultUtil<Notice>(200,n);
    }

    @ApiOperation(value = "create", notes = "可以指定参数的API")
    @PostMapping("/create")
    public JsonResultUtil<Notice> createNotice(String title, String content){
    ...
    @PostMapping("/update")
    public JsonResultUtil<Notice> updateNotice(Long id, String title, String content){
        Integer authorId = (Integer) request.getSession().getAttribute("uid");
        Notice n = noticeService.getById(id);
        n.setAuthorId(authorId);
        n.setTitle(title);
